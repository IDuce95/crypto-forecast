...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"Test MSE: {results.metadata['test_metrics'].mse:.4f}")
        print(f"Test R²: {results.metadata['test_metrics'].r2:.4f}")
        print(f"Best params: {results.best_params}")
        
        if results.feature_importance is not None:
            print("\nTop 5 important features:")
            for i, (feature, importance) in enumerate(results.feature_importance.head().items()):
        ...
        multi_trainer = MultiModelTrainer(["decision_tree", "random_forest"])
        multi_results = multi_trainer.train_all_models(preprocessed_data)
        
        comparison = multi_trainer.compare_models()
        print(comparison[['model', 'test_mse', 'test_r2', 'val_mse', 'val_r2']])
        
        best_model_name, best_model_results = multi_trainer.get_best_model()
