...
# Helm management script for Crypto Forecasting Platform - DEV ENVIRONMENT
# Simplified for single-environment dev deployment
...
VALUES_FILE="values.yaml"  # Use main values file optimized for dev

# Function to print usage
print_usage() {
    echo -e "${BLUE}Helm Management Script - Dev Environment${NC}"
    echo "Usage: $0 {deploy|upgrade|uninstall|status|logs|test|deps|lint|template|values}"
    echo ""
    echo "Commands:"
    echo "  deploy     - Deploy the application to dev cluster"
    ...
    echo "  template   - Render templates for dev"
    echo "  values     - Show effective values"
    echo ""
    echo "Examples:"
    echo "  $0 deploy              # Deploy with default dev values"
...
deploy() {
    echo -e "${BLUE}🚀 Deploying Crypto Forecasting Platform to dev...${NC}"
